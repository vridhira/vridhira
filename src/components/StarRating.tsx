  ...
  return (
    <div className={cn('flex items-center gap-1', className)}>
      {[...Array(fullStars)].map((_, i) => (
        <Star key={`full-${i}`} fill="hsl(var(--foreground))" strokeWidth={0} style={{ width: size, height: size }} />
      ))}
      {halfStar && (
        <div style={{ position: 'relative', width: size, height: size }}>
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, width: '50%', height: '100%', overflow: 'hidden' }}>
            <Star style={{ width: size, height: size }} fill="hsl(var(--foreground))" strokeWidth={0} />
          </div>
        </div>
      )}
